...
be considered almost random decision in practice. As we revisit the
upper bound for the training error for Adaboost \cite{adaboost}, we see
\[
err(H) \leq e^{-2\gamma^2 T}
\]
where $\forall t$, $\gamma_t \geq \gamma $ and $\gamma > 0$, so in our
